<!-- PROFILE_VIEWS_START -->

<div align="center">
<p><img src="https://komarev.com/ghpvc/?username=poliglots&color=7c3aed&style=flat-square&label=Profile+Views" /></p>
<div>

<!-- PROFILE_VIEWS_END -->

<!-- STATS_BANNER_START -->

<div align="left">
<span style="display:inline-block;padding:4px 14px;margin:3px;background:#161b22;color:#e6edf3;border-radius:16px;font-size:13px;line-height:18px;">🗂️ 9 repos</span>
<span style="display:inline-block;padding:4px 14px;margin:3px;background:#161b22;color:#e6edf3;border-radius:16px;font-size:13px;line-height:18px;">💬 76 commits</span>
<span style="display:inline-block;padding:4px 14px;margin:3px;background:#161b22;color:#e6edf3;border-radius:16px;font-size:13px;line-height:18px;">🔀 3 merged PRs</span>
<span style="display:inline-block;padding:4px 14px;margin:3px;background:#161b22;color:#e6edf3;border-radius:16px;font-size:13px;line-height:18px;">🔓 7 open PRs</span>
<span style="display:inline-block;padding:4px 14px;margin:3px;background:#161b22;color:#e6edf3;border-radius:16px;font-size:13px;line-height:18px;">🐛 1 reported issues</span>
</div>

<!-- STATS_BANNER_END -->

<!-- LANGUAGES_STATS_START -->

<div align="left">

### Languages by Commit

```
Java                  33.3% █████████████████
Rust                  33.3% █████████████████
Go                    22.2% ███████████
TypeScript            11.1% ██████
```

</div>

<!-- LANGUAGES_STATS_END -->

<!-- REPO_STATS_START -->

<div align="left">

### Top Repositories Contributed To

```
• etcd-io/etcd ⭐52238 🍴10491 (Go)
• keycloak/keycloak ⭐36638 🍴8902 (Java)
• tikv/tikv ⭐16833 🍴2336 (Rust)
• zitadel/zitadel ⭐14953 🍴1280 (Go)
• apache/shenyu ⭐8831 🍴3066 (Java)
• cloud-hypervisor/cloud-hypervisor ⭐6196 🍴754 (Rust)
• apache/maven ⭐5342 🍴3122 (Java)
• opensearch-project/OpenSearch-Dashboards ⭐2119 🍴1271 (TypeScript)
• rust-vmm/rust-vmm ⭐82 🍴37 (Rust)
```

</div>

<!-- REPO_STATS_END -->

<!-- MERGED_PRS_START -->

<div align="left">

### Recent Merged Pull Requests

🔀 **3** merged PRs across 3 repos

• [#8770](https://github.com/cloud-hypervisor/cloud-hypervisor/pull/8770) — vmm: Set reserve=on by default when hugepages=on
  ↳ *cloud-hypervisor/cloud-hypervisor*

• [#12753](https://github.com/apache/maven/pull/12753) — fix(api-core): properly implement Clock.withZone()
  ↳ *apache/maven*

• [#51635](https://github.com/keycloak/keycloak/pull/51635) — fix: OIDC client registration scope handling auth 
  ↳ *keycloak/keycloak*

</div>

<!-- MERGED_PRS_END -->

<!-- ISSUES_START -->

<div align="left">

### Reported Issues

🐛 **1** issue (1 open · 0 closed) across 1 repo

• [🟢 #52059](https://github.com/keycloak/keycloak/issues/52059) — Flaky test: org.keycloak.testsuite.forms.ResetPass
  ↳ *keycloak/keycloak*

</div>

<!-- ISSUES_END -->


